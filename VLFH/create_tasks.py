...
k = 20
step  = 2
max_lenth = 148 # max number of comments

import os
directory = 'tau='+str(tau)+'_k='+str(k) +'_step='+str(step)
if not os.path.exists(directory):
    os.makedirs(directory)

data = pd.read_csv('raw_text_labeled.csv').iloc[:,1:]
D = pd.DataFrame()
for t in range(int((max_lenth-k)/tau))[:10]:
    # print t
    X = []
    Y = []
    for i in range(204)[:]:
        ...
        for j in range(len_session_i-tau-(k+step*t)+1):
            window = session_i.iloc[j:j+k+step*t]['comment']#.values.flatten().tolist()

            window = ' '.join([x.split('(created at')[0] for x in window])
            X.append(list(extract_textfeatures_flavor2(window))+[i])
            x_task_t = window
            list_y = [1 if x == 'T' else 0 for x in session_i.iloc[j:j + k + t * step + tau]['label'].tolist()]
            Y.append(list_y)
